...
- [Step 10: Entra ID カスタム認証の構成](#step-10-entra-id-カスタム認証の構成)
...
`src/web/staticwebapp.config.json` に `networking` と `forwardingGateway` を追加します:
...
> **注意**: `forwardingGateway.allowedForwardedHosts` の `${PREFIX}` は自分の環境の PREFIX 値に置き換えてください (例: `sampleapp542.japaneast.cloudapp.azure.com`)。
...
### 10-3. SWA App Settings にシークレットを設定
...
### 10-4. staticwebapp.config.json にカスタム認証を追加

`src/web/staticwebapp.config.json` に `auth` セクションを追加します。`<tenant-id>` は 10-2 で取得した `tenant` 値に置き換えてください:
...
> **注意**: `routes`, `responseOverrides`, `navigationFallback` は Step 2 で設定済みのものをそのまま残してください。上記は追加する `auth` セクションのみ抜粋しています。

### 10-5. AppGW Rewrite Rule で X-Forwarded-Host を追加

SWA の認証コールバックが正しく動作するために、AppGW が `X-Forwarded-Host` ヘッダーを転送する必要があります。

```bash
# Rewrite Rule Set の作成
AGW_FQDN="${PREFIX}.japaneast.cloudapp.azure.com"

# az network application-gateway コマンドの api-version 問題を回避するため REST API で直接設定
AGW_ID=$(az network application-gateway show \
  --name "agw-${PREFIX}" \
  --resource-group $RG_NAME \
  --query id -o tsv)
...
# rewriteRuleSets を追加 (jq で設定を注入)
cat /tmp/agw-config.json | jq --arg fqdn "$AGW_FQDN" '
...
' > /tmp/agw-config-updated.json

# ルーティングルールに Rewrite Rule Set を紐づけ
# (httpRule と httpsRule の両方に適用)
cat /tmp/agw-config-updated.json | jq '
  .properties.requestRoutingRules |= map(
    if .properties.httpListener.id then
      .properties.rewriteRuleSet = {
        "id": (.id | split("/requestRoutingRules/")[0] + "/rewriteRuleSets/add-forwarded-host")
      }
    else . end
  )
' > /tmp/agw-config-final.json

# AppGW を更新 (数分かかります)
az rest --method put \
  --url "https://management.azure.com${AGW_ID}?api-version=2024-03-01" \
  --body @/tmp/agw-config-final.json

echo "AppGW Rewrite Rule を設定しました"

# 一時ファイルの削除
rm -f /tmp/agw-config.json /tmp/agw-config-updated.json /tmp/agw-config-final.json
```

> **ポイント**: `az network application-gateway rewrite-rule` コマンドは古い API バージョンを使用してエラーになる場合があります。`az rest` で直接 REST API を呼ぶことで確実に動作します。

### 10-6. 再デプロイと認証フローのテスト
...
**ブラウザでの確認**: `https://${PREFIX}.japaneast.cloudapp.azure.com` に **AppGW 経由**でアクセスし、「Entra ID でログイン」ボタンを押すと Entra ID のサインイン画面にリダイレクトされます。ログイン後はアプリに戻り、認証済み状態で API にアクセスできるようになります。
...
- [ ] カスタム認証 (Entra ID) を構成し、AppGW 経由でログインできることを確認した
...
| 認証はクラウド機能を活用 | マネージド ID (パスワードレス) + SWA Easy Auth (カスタム Entra ID) |
