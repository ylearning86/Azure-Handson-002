...
- [Step 10: Key Vault の作成](#step-10-key-vault-の作成)
- [Step 11: シークレットの保存と RBAC の設定](#step-11-シークレットの保存と-rbac-の設定)
- [Step 12: RBAC ロールの確認と最小特権の原則](#step-12-rbac-ロールの確認と最小特権の原則)
- [Step 13: マネージド ID でアプリからシークレットにアクセス](#step-13-マネージド-id-でアプリからシークレットにアクセス)
- [Step 14: Key Vault の診断ログ有効化](#step-14-key-vault-の診断ログ有効化)
- [Step 15: Microsoft Defender for Cloud の確認](#step-15-microsoft-defender-for-cloud-の確認)
- [Step 16: ネットワークセキュリティの確認](#step-16-ネットワークセキュリティの確認)
...
## Step 13: マネージド ID でアプリからシークレットにアクセス
...
## Step 15: Microsoft Defender for Cloud の確認
...
## (補足) Entra ID カスタム認証の構成

PE 有効環境では SWA の事前構成済み (managed) Entra ID 認証が動作しないため、カスタム認証プロバイダーを構成します。

```bash
# 1. Entra ID にアプリ登録
#    redirect_uri を AppGW ドメインに設定 (PE内にコールバック)
MSYS_NO_PATHCONV=1 az ad app create \
  --display-name "swa-${PREFIX}-auth" \
  --sign-in-audience AzureADMyOrg \
  --web-redirect-uris "https://${PREFIX}.japaneast.cloudapp.azure.com/.auth/login/aad/callback" \
...
`staticwebapp.config.json` に `auth` セクションを追加:
...
> **ポイント**: カスタム認証では `redirect_uri` が AppGW ドメインになるため、認証コールバックが AppGW → PE 経由で SWA に到達します。`identity.*.azurestaticapps.net` を経由しないので、PE 環境でも正常に動作します。
