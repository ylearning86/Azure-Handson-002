...
**確認 2**: KQL でレスポンスタイム遵守率を確認します。

> **重要: KQL 実行前に必ずトラフィックを生成してください**
>
> Application Insights の接続文字列を設定した後、**SWA のアプリページにブラウザでアクセス**して API リクエストを発生させてください。ページを開くと自動で `/api/health` と `/api/infra` が呼び出されます。**複数回リロード**して十分なデータを生成してください。
>
> ```bash
> # AppGW 経由でアクセスしてトラフィックを生成 (5回繰り返し)
> AGW_FQDN="${PREFIX}.japaneast.cloudapp.azure.com"
> for i in $(seq 1 5); do
>   curl -sk "https://${AGW_FQDN}/" -o /dev/null -w "リクエスト $i: HTTP %{http_code}\n"
>   curl -sk "https://${AGW_FQDN}/api/health" -o /dev/null -w "API $i:     HTTP %{http_code}\n"
> done
> ```
>
> トラフィック生成後、**3〜5 分待ってから** KQL を実行してください。Application Insights のデータ取り込みに時間がかかります。

Azure Portal の **Log Analytics ワークスペース** (`law-${PREFIX}-dev`) → **ログ** で以下の KQL を実行し、`compliance_rate` が **80以上** であることを確認します。
...
> **トラブルシューティング: KQL がエラーになる場合**
>
> | エラー内容 | 原因 | 対処 |
> |-----------|------|------|
> | `'AppRequests' is not recognized` | テーブルが未作成 (データ未到着) | SWA の API (`/api/health`) にアクセスし、3〜5 分待ってから再実行 |
> | 結果が 0 件 | 時間範囲内にデータなし | `ago(24h)` を `ago(7d)` に広げて再実行 |
> | `compliance_rate` が `NaN` | `total` が 0 | トラフィックを生成してから再実行 |
...
```bash
# Log Analytics の Customer ID (GUID) を取得
# Note: --workspace には GUID を指定する必要があります (ワークスペース名は不可)
...
# SWA / Functions のログをクエリ (直近24時間)
az monitor log-analytics query \
  --workspace "$LAW_ID" \
  --analytics-query "AppRequests | where TimeGenerated > ago(24h) | project TimeGenerated, Name, DurationMs, ResultCode | order by TimeGenerated desc | take 20" \
  --timespan P1D \
  -o table 2>/dev/null || echo "ログが蓄積されるまで数分かかります。Step 1 でトラフィックを生成してから 3〜5 分お待ちください。"
```

> **注意**: `--workspace` には Log Analytics ワークスペースの **Customer ID** (GUID) を指定します。ワークスペース名 (`law-xxx-dev`) ではエラーになります。
...
Lab 03 Step 7 で `--export-to-resource-specific true` なしで診断設定を作成した場合は、以下のクエリを使用してください。
...
> **トラブルシューティング: KQL がエラーまたは結果 0 件の場合**
>
> - `'AGWAccessLogs' is not recognized` エラー → 診断ログがまだ Log Analytics に到着していません。Lab 03 Step 7 の診断設定が完了しているか確認し、**10〜20 分待ってから**再実行してください。
> - テーブルが `AzureDiagnostics` になっている場合 → Lab 03 Step 7 で `--export-to-resource-specific true` なしで実行した可能性があります。上記の「レガシーモード」のクエリを使用してください。
> - 結果が 0 件 → AppGW 経由でアクセスしてから再実行。`ago(24h)` を `ago(7d)` に広げてみてください。
...
> アクセスログ (`AGWAccessLogs`)、アプリケーションログ (`AppRequests`, `AppTraces`)、操作ログ (アクティビティログ) を組み合わせることで、要件定義の「システムログを取得・保管し出力可能」を実現しています。
